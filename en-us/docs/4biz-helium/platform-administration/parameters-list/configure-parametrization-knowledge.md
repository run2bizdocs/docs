...
| 333 |Enter the interval in minutes to sync the knowledge with the index server.|Between 0 and 59 minutes. Default is empty. This parameter will define the time in minutes that the knowledge will be under revision.|To know the maximum amount of time to input, check [this link](http://www.quartz-scheduler.org/documentation/quartz-2.3.0/tutorials/crontrigger.html), go to "Format" then "Allowed Values".|
