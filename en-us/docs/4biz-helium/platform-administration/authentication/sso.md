...
3.  Copy the file login.conf (validate the information according to the AD) to the folder standalone / configuration (wildfly)::
...
4.  Copy the lightkeytab.keytab file (generated by the command in AD) to the standalone / configuration (wildfly) folder;

5.  Fix standalone.xlm by adding the following information:
...
6.  If you haven't changed JAVA’S jar policy (JCE - Java Cryptography Extension), download the files and put in jdk/jre/lib/security/policy/limited and jdk/jre/lib/security/policy/unlimited;

7.  Import (synchronize) users of LDAP in ITSM.
...
!!! Abstract "ATTENTION"
    ...
    It’s recommended the reading of the following support: https://ss64.com/nt/setspn.html and http://spnego.sourceforge.net/ .
