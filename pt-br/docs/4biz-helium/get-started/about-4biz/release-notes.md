...
## Versão Helium 2.2.7 (2022/09/30)

Bem-vindos ao 4biz Helium Versão 2.2.7. Esta versão apresenta as seguintes correções:
...
|ITSM 10786 |Correção| Correção de erro onde ao logar de visitante para usuário na base de conhecimento acontecia erro de direcionamento de página |
|TASKER 14943 |Correção| Correção de erro de NullPointer - Na hora de fechar um ticket que possui relacionados e já foi reaberto |
|TASKER 14944 |Correção| Correção de erro onde o sistema não estava enviando notificação para partes interessadas, avisando que houve um registro de comentário |
...
Procedimento para receber notificação de e-mail para registro de comentário:
    ...
    1. Cadastrar as partes interessadas no cadastro de conhecimento;
    2. Preencher os parâmetros para envio de e-mail desta funcionalidade:
        SMTP_ENVIO_RemetenteNotificacoesSolicitacao - 10
        ID_MODELO_EMAIL_ENVIO_COMENTARIO_BASE_CONHECIMENTO - 457
    3. Registrar um comentário;
    4. Verificar se um e-mail de notificação de registro de comentário foi enviado.
...
|Número |Tipo|Descrição|
|-------|----|---------|
|ITSM 9627  |Correção| OCS do cliente TJRS não atualiza os ICs|
|ITSM 9610  |Correção| Tratando caracteres de escape do e-mail|
|ITSM 8508	|Correção| Gravar campo motivo no histórico de ticket|
|ITSM 8499	|Correção| O campo motivo não está sendo apresentado no histórico|
|ITSM 9581	|Correção| Mobile: 'formulário de criação' quando estamos criando o ticket, o mesmo fica carregando e não aparece|
|ITSM 9604	|Correção| Mobile: Ao renderizar o formulario builder, o mesmo derruba a sua sessão|
|ITSM 9694	|Correção| Criar opção para informar título na API de criação de ticket|
|ITSM 9534  |Correção| Ao cancelar um ticket via smartportal, apresenta a mensagem "Ticket não cancelado"|
|TASKER 14027 |Correção| Erro na localidade na tarefa Porfolio |
...
!!! warning "ATENÇÃO"
    ...
    Para mais informações sobre o Builder 1.3.7.7, veja o arquivo de [Notas de Release Builder][13]
...
|Número|Tipo|Descrição|
|------|----|---------|
|10975| Correção|	[PostgreSQL / Sql Server] Correção de erro ao adicionar uma imagem no conteúdo do conhecimento para visualização de usuários visitantes.|
|11190| Correção|	[G. Ticket] Correção de erro em que ao suspender um ticket ele sumia da listagem.|
|10979| Correção|	[G. Conhecimento] Correção de erro para o versionamento de conhecimento.|
|11092| Correção|	[G. Ticket] Correção de erro em que ao editar um comentário na tela de ticket não era possível gravar o ticket.|
|11160| Correção|	[G. Ticket] Correção de erro que impedia a criação de um ticket via token.|
|11105| Correção|	[Experience Center] Correção de erro em que ao abrir um ticket pelo centro de experiência ele acrescentava no campo descrição o nome da atividade do portfólio.|
|11106| Correção|	[Experience Center] Correção de erro em que o botão Cancelar ticket não era exibido dentro do ticket selecionado no centro de experiência.|
|11109| Correção|	[G. Conhecimento] Correção de erro em que ao adicionar uma imagem em uma categoria de imagem ou em um conteúdo de conhecimento ele era redirecionado para a tela de login.|
...
|11196| Correção|	[G. Ticket] Correção do erro no componente de fluxo que não podia ser movido no Chrome.|
